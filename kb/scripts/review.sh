...
echo
echo "== 2. 体裁文体 R12–R16 + 成熟度 R18 + 上游声明 R19 + 体裁挂靠 R20（dita-tools lint；draft 记 warning，R18/R20 恒 error）=="
if command -v dita-tools >/dev/null 2>&1; then
  ...
  echo "找不到 dita-tools，R12–R16 / R18 / R19 / R20 未执行（装：scripts/setup-env.sh）" >&2
  lint_skipped=1
fi
...
if [ "$lint_skipped" -ne 0 ]; then
  echo "⚠️  R12–R16 / R18 / R19 / R20 未执行（找不到 dita-tools）"
fi
