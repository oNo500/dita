...
fn clean_topic_passes() {
    ...
    assert_eq!(d.items.len(), 0, "{:?}", d.items.iter().map(|i| i.message()).collect::<Vec<_>>());
}
...
fn curated_violations_are_errors() {
    ...
    assert!(d.error_count() >= 5, "缺节×2 + 旧标签 + 手写日期 + 粗体/程度词: {:?}",
        d.items.iter().map(|i| i.message()).collect::<Vec<_>>());
    assert_eq!(d.warning_count(), 0);
    let msgs: Vec<&str> = d.items.iter().map(dita_diagnostics::Diagnostic::message).collect();
...
fn genre_must_match_dita_type() {
    ...
    assert!(d.items.iter().any(|i| i.message().contains("不能标在 concept 上")));
}
