...
- 四状态：`○` 未建 · `◐` 进行中 · `●` 完成（有全景且零盲区）· `·` 不适用（纯组织节点，如术语库）。符号独立表意，颜色只加速扫描：管道与 `NO_COLOR` 下零转义码。
- **覆盖度沿分类树上卷**：归在 `claude-code` 的内容计入 `coding-agents` 的全景规划——子键是父键的更窄陈述，否则每个叶子都要复制一份近乎相同的全景。
