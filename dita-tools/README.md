...
本项目用 Rust 构建这套能力，从 IA 概览视图出发，逐步演进为支持 Web 编辑器实时预览的完整 DITA 预处理引擎。
...
| `--vocab` | `vocab/subjectScheme.ditamap` | 词表：受控值 + 主题树（应然）+ 对标登记（时间）。读不到就**跳过并说明**，绝不猜 |

**实际输出（`kb` 仓库，2026-08-15 实跑 `--depth 2`）：**

```

知识体系   全库 24 篇（骨架内 24）· 词表规划 82 个主题节点

├── ○ 语言本体   0/4 · 对标 2026-08-08·触发
│   ├── ○ go   —
│   ├── ○ typescript   —
│   ├── ○ javascript   —
│   └── ○ python   —
├── ◐ Web 技术栈   1/6 · 对标 2026-08-08+12mo
│   ├── ○ browsers   —
│   ├── ○ css   —
│   ├── ○ react   —
│   ├── ○ nextjs   —
│   ├── ○ nodejs   —
│   └── ◐ electron   1 篇 · 概览 0/10
├── ○ 数据存储   0/6 · 对标 2026-08-08+12mo
│   ├── ○ postgresql   —
│   ├── ○ redis   —
│   …（其余分支与叶子从略，--depth 控制层数）

○ 未建   ◐ 进行中   ● 完成（有概览且零盲区）   · 不适用   ⚠ 有问题   ⏰ 待复核
需要处理：
  · 2 篇不在任何分支下（只被交付物 map 引用）
  · 维度盲区 26 个
  · @dimension 有 40 个受控值从未被用过
```

**看这份输出该注意什么：**

- `[空]` 与「按分支」是这个工具存在的理由——九个分支里七个是空的、三个有内容的没有概览、唯一一篇 Web 内容还是 draft。这些在 DITA-OT 的任何产物里都看不到（空 map 不产出页面）。
...
- **骨架以词表主题树为骨**，内容挂其上——规划了却没写的子主题是**看得见的一行**，而不是要你自己察觉的缺席。
- 四状态：`○` 未建 · `◐` 进行中 · `●` 完成（有概览且零盲区）· `·` 不适用（纯组织节点，如术语库）。符号独立表意，颜色只加速扫描：管道与 `NO_COLOR` 下零转义码。
- **覆盖度沿分类树上卷**：归在 `claude-code` 的内容计入 `coding-agents` 的概览规划——子键是父键的更窄陈述，否则每个叶子都要复制一份近乎相同的概览。
- 未声明 `domain`（或值不在词表里）的 topic 落进 `⚠ 未归子主题` 桶；填错键**不会**静默消失。
