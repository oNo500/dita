...
- CI 里一条 `dita --project` 顶替一串 for 循环（对比 [08](08-实践建议.md) 的 CI 脚本 —— 那是无项目文件时的写法）
...
schema 管"结构合法"，管不了"team 规定 task 必须有 shortdesc"这类**业务规则**（对照 [00](00-角色与边界.md) 的"schema 管什么不管什么"表）。约束模块能解决其中一部分（结构性收紧），剩下的靠 Schematron —— 它用 XPath 断言表达规则：
...
注意规则里的 context 同样**按 `@class` 匹配** —— 这样规则自动覆盖专门化元素。但这意味着 Schematron 要跑在**能拿到 `@class` 的文档**上：要么经 DTD 校验解析，要么跑在 preprocess 后的中间产物上（见 [07](07-程序化处理.md)）。
...
| DITA-OT 内 | 无内置支持，可自写插件在 preprocess 后挂一步（见 [06](06-DITA-OT插件开发.md) 的 Ant 挂钩） |
