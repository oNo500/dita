...
> [02-复用.md](02-复用.md) 给出的是 key 的操作规则（"先定义者胜"）。本篇讲规范定义的模型。keyscope 与分支过滤叠加时，只有理解模型才能预测结果。
...
如果你在 [07](07-程序化处理.md) 的路上自己实现 key 解析：
...
5. 真的不建议自己实现。用 `dita -f dita` 拿 DITA-OT 解析好的结果，见 [07](07-程序化处理.md)

---

→ 下一步：[11-处理模型.md](11-处理模型.md)
