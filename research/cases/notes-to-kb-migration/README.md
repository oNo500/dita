...
| [`decisions-and-open-items.md`](decisions-and-open-items.md) | **裁定台账与遗留清单**：46 条执行者裁定与 9 条用户裁定（决定、理由、决定错了的代价），以及收尾时仍开放的遗留项（按「合入后近期 / 可长期留 / 待用户裁定」三级分诊） |
...
`decisions-and-open-items.md` 出于同一理由抽出，抽的是另一样东西：全程 55 条裁定与仍开放的遗留项，原本只在 `.superpowers/sdd/…/progress.md` 里，而 `.superpowers/` 在 `.gitignore` 内，工作区一删即失。两份合起来构成"这批内容是怎么来的"的完整档案——一份答"标题依据什么"，一份答"路上做过哪些取舍、还欠什么"。
