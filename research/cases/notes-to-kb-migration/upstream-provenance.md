...
| `engineering-ci` | DITA 工程化与 CI | **本库自造**。①穷尽查证：OASIS 规范只规定语言本身，不讲内容库的工程组织；DITA-OT 4.4 docsrc 全部节点标题为安装、配置与发布操作，无工程约定类节点；社区无对应节点。**拆出 project-files 之后，本篇内容确无上游归处。**②先怀疑切分：三条做法（目录按角色、id 契约、逐变体构建）共用同一前提——内容进版本库之后的组织方式，分开则每条各三行、均不自足。③只组合不发明：「工程化」与 CI 均为软件工程通行词。迁自笔记 08 §工程化建议（笔记 04 §2 的 project file 一节已于 Task 13b 拆出）。 | Task 7 |
