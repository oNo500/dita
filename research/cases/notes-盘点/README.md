...
> [14-元数据与分类策略](../../notes/14-元数据与分类策略.md) 的实战案例：对 `~/code/notes`（Obsidian 库）全量 426 篇笔记做价值分拣。四路并行完成，判据统一。逐文件明细见本目录四份附录。
...
新体系设计见 [../知识体系重塑/](../知识体系重塑/README.md)。

## 与分类法的衔接

分拣所用维度（type / subject / volatility / verdict）与 [14](../../notes/14-元数据与分类策略.md) 定稿的维度直接对应。实测发现两点：**真实主题树比草案细得多**（react / electron / databases / networking…），词表正本已按实际语料修订；库内 `20-06/写作-PKM写作规范` 的原子化判据 + Diátaxis 四象限与本盘点判据同构——原库的写作规范与 DITA 研究殊途同归，其判据精华并入新体系的写作规则，原文件同样不迁移。
