...
| `<map>` | 基础 map（map 类型） |
| `<subjectScheme>` | 受控值表（map 类型），见 [03](03-条件化与分块.md) |
...
> 规范里"文档类型"和"文档类型外壳"也是两个词：**文档类型**是一组模块的组合（抽象），**外壳**是实现该组合的 DTD/RNG 文件（具体）。定义见 [00 第 5 节](00-角色与边界.md)，外壳的作用见 [09](09-架构基础.md)。
...
3. **为凑类型硬拆** —— 拆出无法独立理解的碎片。这违背了更根本的约束：**复用单元的边界是自足性**（见 [12](12-DITA的哲学与原理.md) 推论 2）
...
精确的链接规则（同行不同格才互链、同格默认不链、`targetonly` 单向列）见 [11](11-处理模型.md) 第 5.7 节。
...
| `@chunk` | `combine` / `split`，见 [03](03-条件化与分块.md) |
...
2. simpletable 是**理想的专门化基类**（[05](05-专门化.md) 的 `params` 就从它派生）；CALS 表几乎无法专门化
3. 输出到 Markdown 等弱格式时，合并单元格必然降级（见 [07](07-程序化处理.md) 的映射难点表）—— 源头少用，下游少痛

团队约束里"限制只能用 simpletable"是常见且合理的做法（见 [05](05-专门化.md) 约束一节）。
...
- 引用走 `@href` 或 `@keyref`（key 定义里的 `<keytext>` 自动充当 alt 文本，见 [02](02-复用.md)）
...
- **用不做 schema 校验的方式解析，`@class` 会全部取不到**（见 [07](07-程序化处理.md)）
...
- topicref 属性表中未在上述页面出现的条目（`@processing-role` 等的取值；`@collection-type` 与 `@linking` 已在 [11](11-处理模型.md) 按 topicref 语言参考核对）
