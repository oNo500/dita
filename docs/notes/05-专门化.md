...
规范的正式框架是**三个扩展设施**：文档类型配置、**专门化**、元素类型配置（含约束与扩展两个子类）。完整框架见 [09-架构基础.md](09-架构基础.md)。
...
- **扩展（expansion）** —— 与约束反向，把专门化元素/属性**只加到特定元素类型上而不全局可用**。同属元素类型配置，见 [09](09-架构基础.md)

> ⚠️ 注意层级：结构化 / 域 / 属性是**专门化**这一个设施下的三种，约束与扩展属于**元素类型配置**。把它们摊平并列成"四种（五种）扩展手段"是常见错误。完整框架以 [09](09-架构基础.md) 为准。

**无论用哪一种，最终都要靠 document-type shell 装配进来** —— shell 是所有定制的汇合点，见 [09](09-架构基础.md)。
...
2. **值由 doctype shell 的默认值注入**，作者不手写 —— 和 `@class` 一样，**裸解析拿不到**（见 [07](07-程序化处理.md)）
...
**RNG 写好后怎么给下游生成 DTD**（[07](07-程序化处理.md) 的 lxml 路线等场景仍需要 DTD）：
...
- 前提是你的 RNG 遵守了 [00](00-角色与边界.md) 提到的那条限制：**不使用无法翻译成 DTD 构造的 RELAX NG 特性**——OASIS 的模块刻意如此，你照着模板写自然满足
...
→ 下一步：[06-DITA-OT插件开发.md](06-DITA-OT插件开发.md)
