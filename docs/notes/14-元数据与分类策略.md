...
> 最佳实践层第一篇。[03](03-条件化与分块.md) 讲了用于**过滤**的元数据（条件属性），本篇讲用于**分类与检索**的元数据：字段放哪、受控值怎么设计、分类树怎么画。
>
...
| Source | `<source>` | |
| Language | `@xml:lang`（见 [13](13-翻译与本地化.md)） | |
...
|---|---|---|---|---|---|
| **条件属性**（`@props` 专门化） | ✅ subjectScheme | ✅ | ✅（[11](11-处理模型.md) 级联清单） | 过滤 / flag | **影响交付物内容**的维度 |
...
[03](03-条件化与分块.md) 把 subjectScheme 当"条件值的拼写检查"介绍，那只是它的最小用法。它实际是一个**受控词表 + 分类树**机制，规范语义如下（本节已逐页核对）：
...
- `<defaultSubject>` 提供的默认值就是 [11](11-处理模型.md) 五级优先级的**第 4 级**（显式值、语法默认、级联值之后；处理器默认之前）
...
**一棵树管一个正交维度**（[12](12-DITA的哲学与原理.md) 推论 5 在分类法上的对应物）：平台一棵、受众一棵、产品一棵。想把多个维度编进一棵树的冲动，就是组合爆炸的开始。
...
[08](08-实践建议.md) 看好的方向在这里落地：topic 是自足的检索单元（[12](12-DITA的哲学与原理.md) 推论 2），本篇的元数据就是它的检索元数据 —— `@class` 给出类型、`@subjectrefs` 给出主题分面、prolog 给出作者与时效、subjectScheme 树本身就是现成的分面层级。做 RAG 分块时这些全部现成可用，前提是第 5 节的流程真的执行了 —— **失真的元数据喂给检索器，比没有元数据更糟**。
...
- [绑定受控值到属性](https://dita-lang.org/2.0/dita/archspec/base/binding-controlled-values-to-attribute) — `<enumerationdef>` / `<attributedef>` / `<subjectdef>` / `<defaultSubject>` 的分工；**容器节点本身不是合法值**；**空枚举使属性无合法值**；处理器 SHOULD 按受控集校验；受控值默认在显式值、语法默认、级联值之后、处理器默认之前求值（与 [11](11-处理模型.md) 五级优先级第 4 级吻合）
