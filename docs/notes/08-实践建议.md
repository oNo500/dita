...
变体矩阵一旦变大（多 transtype × 多 ditaval），把这串 for 循环换成 **DITA-OT 项目文件**（`dita --project`，见 [04](04-工具链与构建.md)）—— 交付物声明一次，CI 里一条命令。
...
- **结构化内容 + LLM**：topic 是语义完整的检索单元，`@class` 与条件属性提供现成的结构化元数据 —— 完整论证与工程决定（解析形态、分库还是检索时过滤）见 [15-DITA与RAG](15-DITA与RAG.md)
...
| 主题 | 位置 |
|---|---|
| 条件处理的规范语义（判定逻辑 / flag 优先级 / passthrough） | [03-条件化与分块](03-条件化与分块.md) |
| 目录（TOC）生成规则、索引设施 | [11-处理模型](11-处理模型.md) |
| 翻译与本地化（`@xml:lang` / `@dir` / `@translate`、复用与翻译的反模式） | [13-翻译与本地化](13-翻译与本地化.md) |
| subjectScheme 的规范语义（沿层级向上查找 / 绑定 / defaultSubject） | [03](03-条件化与分块.md) + [14](14-元数据与分类策略.md) |
| 链接生成（`@collection-type` / `@linking` / reltable 规则） | [11](11-处理模型.md) |
| DITA 与 RAG（切块 / 元数据 / 解析形态的选择） | [15-DITA与RAG](15-DITA与RAG.md) |
| 元数据放在哪、Dublin Core 对应、分类树设计 | [14-元数据与分类策略](14-元数据与分类策略.md) |
| 文档类型外壳（document-type shell） | [09-架构基础](09-架构基础.md) |
| 扩展模块（expansion module） | [09](09-架构基础.md) |
| 三大扩展设施的正式框架 | [09](09-架构基础.md) |
| Conformance —— 一致的处理器/文档/shell | [09](09-架构基础.md) |
| 寻址体系（直接 vs 间接、片段标识符两种语法） | [10-寻址与键空间](10-寻址与键空间.md) |
| Key space 的正式模型、`@keyscope` 嵌套、跨交付物寻址 | [10](10-寻址与键空间.md) |
| **分支过滤 × 键空间**（含"必须先过滤再建键空间"的顺序约束） | [10](10-寻址与键空间.md) |
| **确定属性有效值**的五级优先级 | [11-处理模型](11-处理模型.md) |
| 元数据级联的完整属性清单与 `@cascade` merge/nomerge | [11](11-处理模型.md) |
| conref 解析时的属性规则与 `-dita-use-conref-target` | [11](11-处理模型.md) |
| 排序与 `<sort-as>`（含中文注音排序键） | [11](11-处理模型.md) |

### 剩余缺口

**处理模型** —— 已全部覆盖（TOC / 索引 / 链接生成在 [11](11-处理模型.md)，条件处理在 [03](03-条件化与分块.md)，subjectScheme 在 [03](03-条件化与分块.md)/[14](14-元数据与分类策略.md)）。层级链接的具体生成属实现惯例，规范未单列算法。
...
- **无障碍（Accessibility）**（**可选，仅记录**）—— 规范有独立章节；目前仅 [01](01-核心模型.md) 3.6 节的 `<alt>` 一句带过，暂不展开
...
**用语法文件校验示例（规范层，可独立先行）** —— 示例文件用 **v2.0-beta03 自带的 RNG/DTD** 直接校验（jing / xmllint + OASIS catalog，不经 DITA-OT）。语法文件是规范的组成部分（RNG 为规范性版本，见 [00](00-角色与边界.md)），这样校验示例合法性仍属规范层验证，不掺入任何实现的行为。已修正的 prolog 顺序类错误由这一层兜住。

**用 DITA-OT 构建验证实现行为（实现层，待工具链规划启动后随之进行）** —— 可构建的示例工程只能验证本来就标注"来自实现观察"的断言：preprocess 流水线顺序（[04](04-工具链与构建.md)）、`@class` 模块名实际取值（[06](06-DITA-OT插件开发.md)）、lwdita 对 2.0 元素的覆盖（[07](07-程序化处理.md)）、`dita init` 模板名（README）。其结论只进"实现观察"档，**不得回写规范层内容**；构建结果与规范冲突时，默认解释是基线滞后而非笔记有错（见本篇踩坑表）。

**其他待办**：无障碍降为**可选项，仅保留记录**，暂不展开。（lwdita 的 TypeScript 实现已核实并更新进 [07](07-程序化处理.md)；LLM/RAG 已成篇为 [15](15-DITA与RAG.md)。）
