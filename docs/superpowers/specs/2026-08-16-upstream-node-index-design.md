...
**执行修正（2026-08-18，T4 完成）**：上面这句里的两个前提都不成立，记下来备后续分支推广时参考。

- **对照表不是依据，索引才是。** 抽出的对照表有八处经核实不成立（1.3 章名、map 标题误当 topic 节点、`<dt>` 术语误当节点名、凭文件名反推标题），逐篇拿标题与正文回索引核对才定得下声明。清单见 `research/cases/notes-to-kb-migration/upstream-provenance.md` 的「附之零」。
- **三道关说明不是都在头注释里。** 30 篇标自造的里有 7 篇只有散文式查证记录、无三关标注，得补写；另有 6 篇根本不该是自造——两端在索引里都有节点，改判为组合篇后声明多条，漂移才报得出来。
- 执行顺序改为**先 T5 后 T4**：先有机器即时反馈，写 66 条声明时错了当场知道。无检查地手写 66 条，错误会静默留下。

最终分诊：1 个节点 28 篇 / 2 个以上 14 篇 / `coined` 24 篇，共 66 篇（设计稿写的 65 是 Task 10b 收口时的数）。
