...
① 平台实现 + 测试；② 与旧脚本**差分对账**通过（同一数据，结果一致或差异逐条解释）；
③ 删除脚本（不留僵尸）；④ 同步能力地图本表、kb README、Justfile；⑤ `just check` 全过。
另：**不建 `tools/` 伞目录**——那是给注定消失的东西修候车室。`kb/` 的「纯声明性」不变式（目录里只有 XML/RNG/DITAVAL，无一行可执行代码）是吸收完成后的终态，届时 kb 这个名字与内容完全相符。过渡期的统一入口由根 `Justfile` 承担，使用者无需知道哪条命令是脚本、哪条是平台。
...
| ~~3~~ | ~~环境是否可复现~~ | **✅ 已决并落地（2026-08-15）**：`scripts/setup-env.sh` 入仓库，幂等，工具版本号以其顶部为 SSOT（别处不许再抄版本）；`just setup` 一键引导 | 换机器 = clone + setup + check |
